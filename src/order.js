(function() {
  ...
  kintone.events.on('app.record.detail.show', function(event) {
    ...
    var managers = [];
    var i;
    var myContainer = document.querySelector(
      '.gaia-app-statusbar-actionmenu-wrapper'
    );
    var amount =
      Number(record.发货金额合计.value) - Number(record.修改前实际金额.value);
        ...
        return kintone
          ...
          .then(function(resp1) {
            ...
            var recordCustomer = records.find(function(item) {
              return (
                item.年月.value === yearMonth && item.汇总区分.value === '客户'
              );
            });
            var recordSingleRegion = records.find(function(item) {
              return (
                item.年月.value === yearMonth &&
                item.汇总区分.value === '各区域'
              );
            });
            ...
            var sumYearSingleRegionOut = getSumOut(
              recordSingleRegion,
              records,
              '各区域'
            );
            var sumYearAllRegionOut = getSumOut(
              recordAllRegion,
              records,
              '全区域'
            );
            var sumYearCustomerIn = getSumIn(recordCustomer, records, '客户');
            var sumYearSingleRegionIn = getSumIn(
              recordSingleRegion,
              records,
              '各区域'
            );
            var sumYearAllRegionIn = getSumIn(
              recordAllRegion,
              records,
              '全区域'
            );
            ...
            var recordsUpdate =
              category === '销售订单' ? recordsOut : recordsIn;
        ...
        kintone.api(
          ...
          function(resp) {
            ...
            for (m = 0; m < tables.length; m++) {
              ...
              optionStr +=
                '>' + tables[m].value.单行文本框_7.value + '</option>';
            }
        ...
        $('#receivingAddress').change(function() {
          ...
          kintone.api(
            ...
            function(resp) {
              ...
              var n;
              if (
                $('#receivingAddress option:selected').text() ===
                '请选择收货地址'
              ) {
                ...
                for (n = 0; n < table2.length; n++) {
                  if (
                    $('#receivingAddress option:selected').text() ===
                    table2[n].value['单行文本框_7'].value
                  ) {
                    record2.record['收货人'].value =
                      table2[n].value['单行文本框_8'].value;
                    record2.record['电话'].value =
                      table2[n].value['单行文本框_9'].value;
                    record2.record['订货员'].value =
                      table2[n].value['单行文本框_10'].value;
                    record2.record['送货地址'].value =
                      table2[n].value['单行文本框_7'].value;
                    break;
                  }
    ...
    if (event.nextStatus.value === '完成') {
      return kintone
        ...
        .then(function(resp1) {
          ...
          var recordSingleRegion = records.find(function(item) {
            return (
              item.年月.value === yearMonth && item.汇总区分.value === '各区域'
            );
          });
          var recordAllRegion = records.find(function(item) {
            return (
              item.年月.value === yearMonth && item.汇总区分.value === '全区域'
            );
          });
          var sumYearCustomerOut = getSumOut(recordCustomer, records, '客户');
          var sumYearSingleRegionOut = getSumOut(
          ...
          var recordsOut = [
            {
              ...
              record: {
                ...
                当月订单额: {
                  value:
                    Number(recordCustomer.当月订单额.value) + deliveryAmount,
                },
              },
            },
            {
              ...
              record: {
                ...
                当月订单额: {
                  value:
                    Number(recordSingleRegion.当月订单额.value) +
                    deliveryAmount,
                },
              },
            },
            {
              ...
              record: {
                ...
                当月订单额: {
                  value:
                    Number(recordAllRegion.当月订单额.value) + deliveryAmount,
                },
              },
            },
          ];
  ...
  kintone.events.on(
    ['app.record.create.change.订单数量', 'app.record.edit.change.订单数量'],
    function(event) {
  ...
  kintone.events.on(
    ['app.record.create.change.订单明细', 'app.record.edit.change.订单明细'],
    function(event) {
      var type = event.type;
      var record = event.record;
      var table = record.订单明细.value;
      ...
      for (i = 0; i < table.length; i += 1) {
        if (
          ...
          (type === 'app.record.edit.change.订单明细' &&
            record.状态.value === '未处理')
        ) {
        ...
        kintone.api(
          ...
          function(resp) {
            ...
            for (i = 0; i < table.length; i++) {
              ...
              optionStr +=
                '>' + table[i].value['单行文本框_7'].value + '</option>';
            }
            $('#receivingAddress').append(optionStr);
            // 默认选择
            $('#receivingAddress')
              .find('option:contains(\'请选择收货地址\')')
              .attr('selected', true);
          },
          function(error) {
            // error
            ...
            function(resp) {
              ...
              if (
                ...
                for (i = 0; i < table.length; i++) {
                  if (
                    $('#receivingAddress option:selected').text() ===
                    table[i].value['单行文本框_7'].value
                  ) {
                    record2.record['收货人'].value =
                      table[i].value['单行文本框_8'].value;
                    record2.record['电话'].value =
                      table[i].value['单行文本框_9'].value;
                    record2.record['订货员'].value =
                      table[i].value['单行文本框_10'].value;
                    record2.record['送货地址'].value =
                      table[i].value['单行文本框_7'].value;
                    break;
                  }
